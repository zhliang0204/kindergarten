...
export default class ChildBindWithMother extends Component {
  constructor(props){
    super(props)
    this.state = {
  ...
  skipMother(e){
    ...
    this.props.info.history.push("/chlidrenList")
  }

  bindMother(e){
    e.preventDefault();
    e.stopPropagation();
    api.getAllParentAccordToRole("mother")
        .then(parents => {
          let parentsList = []
          parents.map((cur,i) => {
            let childname = ""
        ...
        {this.props.langTab ==="lang4" && (<div className="english">
         <div className="parent">
            ...
            {this.state.isBind && (<div>
              <Form>
              <FormGroup>
                  <Label for="parent">Choose Mother:</Label>
            ...
            {/* {this.state.curParent !== "" && (<div>
              <div>first name</div> <div>{this.state.curParent.firstname}</div>
              <div>last name</div> <div>{this.state.curParent.lastname}</div>
              <div>Mobile</div> <div>{this.state.curParent.phone}</div>
              <div>Email</div> <div>{this.state.curParent.email}</div>
            </div>)} */}
            ...
            <div style={{textAlign:"right"}}>
                <button className="btn-click"  onClick={(e)=>this.handleParentBindSubmit(e)}><i className="fas fa-arrow-right"></i>Submit</button>
            </div>
            </div>)}
            ...
            {this.state.isCreate && (<div>
              <Form>
                ...
                <div style={{textAlign:"right"}}>
                  <button className="btn-click"  onClick={(e)=>this.handleParentSubmit(e)}><i className="fas fa-arrow-right"></i>Submit</button>
                </div>
            </Form>
